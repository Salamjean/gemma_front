  ...
  Swal.fire({
      ...
      <div class="text-left space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select 
              id="swal-doctor" 
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${loadingDoctors ? 'opacity-50' : ''}"
            ...
            ${loadingDoctors ? '<p class="text-xs text-gray-500 mt-1">Chargement des médecins...</p>' : ''}
          </div>
        </div>
    ...
    preConfirm: () => {
      ...
      if (image && image.size > 2 * 1024 * 1024) {
        Swal.showValidationMessage('Le fichier est trop volumineux (max 2MB)');
        return false;
      }
    ...
    if (result.isConfirmed && result.value) {
      const { title, date, heure, doctorId, motif, notes, image } = result.value;
      await createRendezVous(title, date, heure, doctorId, motif, notes, image);
    }
  });
};

  const createRendezVous = async (title, date, heure, doctorId, motif, notes, image) => {
    const token = localStorage.getItem('patient_token');
    ...
    try {
      ...
      if (response.ok) {
        ...
        const doctorName = selectedDoctor ? `Dr. ${selectedDoctor.name}` : 'le médecin';
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900 flex items-center">
                <FaCalendarAlt className="mr-3" style={{ color: PRIMARY_BLUE }} />
                Mes Rendez-vous
              </h1>
            ...
            <div className="flex gap-3">
              <button 
                ...
                className={`px-4 py-2 ${loadingDoctors ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'} text-white rounded-lg transition text-sm font-medium flex items-center`}
                style={{ backgroundColor: loadingDoctors ? '#94a3b8' : PRIMARY_BLUE }}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <StatCard
              title="Terminés"
              value={rendezVous.filter(rdv => rdv.status === 'complete').length}
              color="#6b7280"
              icon={FaFileMedical}
            />
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {filteredRendezVous.map((rdv) => {
              ...
              return (
                <div key={rdv.id} className="bg-white rounded-xl border border-gray-200 hover:shadow-lg transition-shadow p-6">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          <div className="flex items-center text-sm text-gray-600 mb-1">
                            ...
                            <span>{rdv.doctor_name || 'Médecin non spécifié'}</span>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-900">{rdv.motif || 'Non spécifié'}</p>
                        </div>
                      </div>
                    </div>
